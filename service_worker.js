...
function aggregateScanResults(analyzedResults) {
  ...
  analyzedResults.forEach(res => {
    ...
    if (res.platformEvidence && res.platformEvidence.length > 0) {
      finalResult.platformEvidence.push(...res.platformEvidence);
    }
  ...
  if (platformsFound.size > 1 || platformsFound.has('Multiple')) {
  ...
  finalResult.platformEvidence = [...new Set(finalResult.platformEvidence)];

  if (enginesFound.length > 0) {
    enginesFound.sort((a, b) => b.confidenceScore - a.confidenceScore);
...
function analyzeProbeResult(raw) {
  ...
  // 1. Detect Platforms
  const platformResult = detectPlatforms(meta);
  results.adPlatform = platformResult.adPlatform;
  results.platformEvidence = platformResult.platformEvidence;
...
async function performProbe(fetchTimeoutMs, scanId, tabId) {
  const meta = {
    ...
    resources: typeof performance !== 'undefined' ? performance.getEntriesByType('resource').map(r => r.name) : [],
